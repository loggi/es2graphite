...
SHARD_STATE = {
    'CREATED': 0, 'RECOVERING': 1, 'STARTED': 2, 'RELOCATED': 3, 'CLOSED': 4}
HOST_IDX = -1
...
def add_metric(metrics, prefix, stat, val, timestamp):
    ...
    if prefix[-1] == 'translog' and stat == 'id':
        ...
        metrics.append(
            (normalize((prefix, stat)), (timestamp, SHARD_STATE[val])))
...
def process_node_stats(prefix, stats):
    ...
    for node_id in stats['nodes']:
        ...
        process_section(int(time.time()), metrics,
                        (prefix, CLUSTER_NAME, NODES[node_id]), node_stats)
    return metrics


def process_cluster_health(prefix, health):
    metrics = []
    process_section(int(time.time()), metrics, (prefix, CLUSTER_NAME),
                    health)
    return metrics
...
def process_indices_status(prefix, status):
    metrics = []
    process_section(int(time.time()), metrics,
                    (prefix, CLUSTER_NAME, 'indices'), status['indices'])
    return metrics
...
def process_indices_stats(prefix, stats):
    metrics = []
    process_section(int(time.time()), metrics,
                    (prefix, CLUSTER_NAME, 'indices', '_all'),
                    stats['_all'])
    process_section(int(time.time()), metrics,
                    (prefix, CLUSTER_NAME, 'indices'), stats['indices'])
    return metrics
...
def process_section(timestamp, metrics, prefix, section):
    for stat in section:
        ...
        if 'timestamp' in section:
            timestamp = int(section[
                                'timestamp'] / 1000)  # es has epoch in ms,
                                # graphite needs seconds
        ...
        if isinstance(stat_val, dict):
            ...
            if prefix[-1] == 'fs' and stat == 'data':
                for disk in stat_val:
                    pass  # mount = disk['mount']
                    # process_section(timestamp, metrics, (prefix, stat,
                    # mount), disk)
            elif prefix[-1] == 'os' and stat == 'load_average':
                add_metric(metrics, prefix, (stat, '1min_avg'), stat_val[0],
                           timestamp)
                add_metric(metrics, prefix, (stat, '5min_avg'), stat_val[1],
                           timestamp)
                add_metric(metrics, prefix, (stat, '15min_avg'),
                           stat_val[2], timestamp)
            elif prefix[-1] == 'shards' and re.match('\d+',
                                                     stat) is not None:
                for shard in stat_val:
                    shard_node = NODES[shard['routing']['node']]
                    process_section(timestamp, metrics,
                                    (prefix, stat, shard_node), shard)
            else:
                for stat_idx, sub_stat_val in enumerate(stat_val):
                    if isinstance(sub_stat_val, dict):
                        process_section(timestamp, metrics,
                                        (prefix, stat, str(stat_idx)),
                                        sub_stat_val)
                    else:
                        add_metric(metrics, prefix, (stat, str(stat_idx)),
                                   sub_stat_val, timestamp)
        else:
            add_metric(metrics, prefix, stat, stat_val, timestamp)


def send_to_graphite(metrics):
    if args.mode == MODE_CARBON_PICKLE:
        send_to_carbon_pickle(metrics)
    for m, mval in metrics:
        if args.mode == MODE_STATSD:
            send_to_statsd(m, mval[1], mval[0])
        if args.debug:
            send_to_log(m, mval[1], mval[0])


def send_to_log(metric_name, value, timestamp):
    log('%s %s = %s' % (timestamp, metric_name, value), True)


def send_to_carbon_pickle(metrics):
...
def get_metrics():
    ...
    cluster_health_url = 'http://%s/_cluster/health?level=%s' % (
    get_es_host(), args.health_level)
    ...
    if args.segments:
        ...
        segments_status_metrics = process_segments_status(args.prefix,
                                                          segments_status)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Send elasticsearch metrics to graphite')
    parser.add_argument('-p', '--prefix', default='es',
                        help='graphite metric prefix. Default: %(default)s')
    parser.add_argument('-g', '--host', default='localhost',
                        help='graphite hostname. Default: %(default)s')
    parser.add_argument('-o', '--port', default=2004, type=int,
                        help='graphite pickle protocol port. Default: %('
                             'default)s')
    parser.add_argument('-i', '--interval', default=60, type=int,
                        help='interval in seconds. Default: %(default)s')
    parser.add_argument('-m', '--mode', default='carbon-pickle',
                        help='Which mode to use: carbon-pickle, statsd')
    parser.add_argument('--health-level',
                        choices=['cluster', 'indices', 'shards'],
                        default='indices',
                        help='The level of health metrics. Default: %('
                             'default)s')
    parser.add_argument('--shard-stats', action='store_true',
                        help='Collect shard level stats metrics.')
    parser.add_argument('--segments', action='store_true',
                        help='Collect low-level segment metrics.')
    parser.add_argument('-d', '--debug', action='store_true',
                        help='Print metrics, don\'t send to graphite')
